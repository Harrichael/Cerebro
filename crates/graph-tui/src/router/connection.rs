...
#[derive(Debug, Clone)]
pub struct ConnectionsLayout<'a> {
...
impl<'a> ConnectionsLayout<'a> {
	...
	/// Connections `calculate` could not find a route for. Check this: the
	/// number of connections pushed says nothing about how many were drawn,
	/// and an unroutable edge leaves no mark on the canvas at all.
	pub fn diagnostics(&self) -> &[Diagnostic] {
		&self.diagnostics
	}

	pub fn push_connection(&mut self, connection: (Connection<'a>, usize)) {
	...
	pub fn block_port(&mut self, coord: (usize, usize), vertical: bool) {
	...
	pub fn calculate(&mut self, direction: super::FlowDirection) {
			...
			for x in 0..self.width {
				...
				let (symbol, line_style) = match (north, south, east, west) {
					...
					(n, s, e, w) if n == B || s == B || e == B || w == B => {
						...
						// Upstream's `e == B && w == B && n == s` case used to sit
						// here; the general north branch below subsumes it, and
						// the one shape it caught that this does not (n == B) is
						// already taken by the all-blocked arm above.
						} else if n != E && n != B {
							(bor(n).vertical, get_line_style(n))
						} else if s != E && s != B {
							(bor(s).vertical, get_line_style(s))
						} else if e != E && e != B {
							(bor(e).horizontal, get_line_style(e))
						} else if w != E && w != B {
							(bor(w).horizontal, get_line_style(w))
						} else {
							continue;
						}
					}
					...
					// Two different connections meeting in one cell match none
					// of the patterns above, and upstream drew "?" there. A
					// code graph crosses constantly, so prefer the vertical run
					// and let the horizontal pass beneath it -- the same choice
					// the equal-connection crossing case makes just above.
					(n, s, _, _) if n != E && s != E => (bor(n).vertical, get_line_style(n)),
					(_, _, e, w) if e != E && w != E => (bor(e).horizontal, get_line_style(e)),
					(n, _, _, _) if n != E => (bor(n).vertical, get_line_style(n)),
					(_, s, _, _) if s != E => (bor(s).vertical, get_line_style(s)),
					(_, _, e, _) if e != E => (bor(e).horizontal, get_line_style(e)),
					(_, _, _, w) if w != E => (bor(w).horizontal, get_line_style(w)),
					(_, _, _, _) => continue,
				};
